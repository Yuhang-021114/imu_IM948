...
static void Cmd_RxUnpack(U8 *buf, U8 DLen)
{
    ...
    case 0x11: // 获取订阅的功能数据 回复或主动上报
        {
            sensor_msgs::msg::Imu imu_data;//要发布的IMU数据
            imu_data.header.stamp = rclcpp::Clock(RCL_SYSTEM_TIME).now();
            ...
            extern std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::Imu>> imuMsg_pub;
...
int UART_Write(const U8 *buf, int Len) {
    ...
    if (!ser.isOpen()) {
        ser.setPort("/dev/ttyUSB0");  // 修改为你的串口设备
        ser.setBaudrate(115200);
        serial::Timeout to = serial::Timeout::simpleTimeout(1000);
        ser.setTimeout(to);
        ser.open();
    }
    return ser.write(buf, Len);
}
